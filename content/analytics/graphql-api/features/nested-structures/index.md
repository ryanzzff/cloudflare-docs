...
- Some arrays contain primitive types (e.g. `[String]`) and function like ordinary fields that [can be filtered](/analytics/graphql-api/features/filtering/)
- Some arrays contain more complex types (e.g. `[Subrequest]`.) The following section describes their behaviour.

Arrays of non-primitive types behave as a special kind of single value. There is no way to paginate through, filter, filter by, group, or group by the array.
